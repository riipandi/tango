...
// DBCmd groups the database backup and restore operations ported
// from scripts/migrator.sh. A development tool: it shells out to
// the local pg_dump/pg_restore/psql binaries and writes to the
...
// dbOptions registers the db command group into the CLI grammar
// (debug builds only).
func dbOptions() []kong.Option {
	return []kong.Option{
		kong.DynamicCommand("db", "Database backup and restore", "", &DBCmd{}),
	}
}
