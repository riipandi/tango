...
- The signed body is exactly the delivered body and can be verified independently. Store immutable
  canonical delivery bytes rather than re-serializing JSONB for retries.
...
2. Keep endpoint configuration, event subscriptions, deliveries, and attempts in separate focused
   stores and services. Store signing secrets with `pkg/crypto` using `enc:` and ensure secrets
   never appear in list/get responses. Commit:
   `refactor: simplify webhook boundaries`.
3. Harden deterministic HMAC signing over immutable body bytes, timestamp handling, bounded
   timeouts, idempotent delivery/attempt records, queue delivery, retries, and pruning. Test
   tampering, timeout, non-2xx, duplicate delivery, and rotation. Commit:
   `fix: harden webhook delivery`.
