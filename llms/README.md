...
| ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `/healthz`, `/api/version/current`, `/api/version/latest`                                                                    | deviation                                                                  | tango serves `/api/healthz` (200); version routes absent — add in phase 5 cleanup                    |
| `/api/users` GET/POST, `/api/users/{id}` GET/PUT/DELETE, `/api/users/{id}/groups`                                            | ok                                                                         | phase 1–2                                                                                            |
| `/api/users/me` GET/PUT (+ profile-picture)                                                                                  | missing-backend                                                            | phase 5 (alias `/api/account` exists as tango extension)                                             |
| `/api/users/{id}/user-groups` PUT                                                                                            | missing-backend                                                            | phase 5 (group membership only editable from group side today)                                       |
...
| `/api/users/{id}/one-time-access-email                                                                                       | token`, `/api/one-time-access-email`, `/api/one-time-access-token/{token}` | missing-backend                                                                                      | phase 5                                                                                                                                           |
...
| `/api/user-groups/{id}/allowed-oidc-clients` PUT                                                                             | missing-backend                                                            | phase 5 (allowlist editable only from the client side today)                                         |
| `/api/custom-claims/suggestions`, `/api/custom-claims/user                                                                   | user-group/{ownerId}`                                                      | deviation                                                                                            | phase 2 uses single-claim POST/PUT/DELETE; upstream replaces the claim list with one PUT — keep single-claim API, add list-replace PUT in phase 5 |
| `/api/audit-logs` (self), `/api/audit-logs/all` (admin), `/filters/users                                                     | client-names`                                                              | deviation                                                                                            | phase 2 mounts the admin list at `/api/audit-logs` with filters; add `/all` alias + per-user self listing in phase 5                              |
| `/.well-known/jwks.json`, `/.well-known/openid-configuration`                                                                | ok                                                                         | phase 3                                                                                              |
| `/.well-known/oauth-authorization-server`                                                                                    | missing-backend                                                            | phase 5 (RFC 8414 mirror of the discovery doc)                                                       |
| `/api/oidc/clients` GET/POST, `/api/oidc/clients/{id}` GET/PUT/DELETE                                                        | ok                                                                         | phase 4 (fields may lag: logo, credentials list)                                                     |
| `/api/oidc/clients/{id}/allowed-user-groups` PUT                                                                             | missing-backend                                                            | phase 5 (allowlist editable only via client PUT payload today)                                       |
| `/api/oidc/clients/{id}/secrets*` (list/create/delete multi-secret)                                                          | missing-backend                                                            | phase 5 (single secret today)                                                                        |
| `/api/oidc/clients/{id}/logo*`, `/api/oidc/clients/{id}/meta`, `/preview/{userId}`, `/refresh` (CIMD)                        | missing-backend                                                            | phase 8 (logos/storage), phase 5 (meta/preview), phase 8 (CIMD)                                      |
| `/api/oidc/token`, `/api/oidc/userinfo`                                                                                      | ok                                                                         | phase 4                                                                                              |
| `/api/oidc/introspect`                                                                                                       | missing-backend                                                            | phase 5 (form-encoded RFC 7662)                                                                      |
| `/api/oidc/users/me/clients`, `/api/oidc/users/me/authorized-clients*`, `/api/oidc/users/{id}/authorized-clients`            | missing-backend                                                            | phase 5 (consent records exist in `user_authorized_oidc_clients`)                                    |
| `/authorize` (authorize code + PKCE), `/api/oidc/end-session`                                                                | deviation                                                                  | authorize ok (phase 4); end-session route exists as a discovery constant only — implement in phase 5 |
...
Yaak folder coverage after phase 4: Users, User Groups, Custom Claims, Audit Logs, Well Known,
OIDC, OAuth, Tango Extensions (auth) exist with the implemented requests; missing _requests_
inside existing folders mirror the `missing-backend` rows above and get created as each phase
lands. Version folder is absent — create it when the version routes land.
