// Package queue adapts the antree task queue to the kernel module
// lifecycle: Start runs the dispatcher, Stop waits for workers to
// drain. Queue registration happens at build time via registry.Deps.
package queue
...
type Module struct {
	client *antree.Client
}

// New wraps a client for kernel registration.
func New(client *antree.Client) *Module {
