...
- `internal/static` — serves `storage.local_path/uploads` at `/static`, outside the throttled group and before the SPA so a missing upload is a 404 rather than `index.html`. `os.DirFS` resolves the request path, so a `..` segment is refused by the filesystem. It is deliberately not the chunk store: the engine writes content-addressed chunks under `chunks/`, which is never served; a feature that wants a URL writes a whole copy under `uploads/`.
