...
- `task key:generate` runs `go run -tags debug ./cmd key:generate --env-file=.env.local --overwrite`; `task db:migrate` runs `migrate:up`, `db:rollback` runs `migrate:down`, `db:status`/`db:version` wrap the matching commands, `db:reset` runs `migrate:reset`, and `db:validate` runs `migrate:validate`. Pass extra flags after `--`.
...
- Add a migration: create `database/migrations/<NNNNN>_<name>.sql` with goose Up/Down blocks, then run `task db:migrate`. The version must be `>= 1`, exactly 5 digits, consecutive with the previous file, and higher than the last. Migrations are embedded in the binary, so a new file only runs after a rebuild. Run `task db:validate` to check the files without a database.
