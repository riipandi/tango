...
- `modules/identity/jwks` — the published key set at `GET /.well-known/jwks.json`: the configured key pair (`auth.private_key`/`auth.public_key`, read with `crypto.DecodeJWK`) plus the active signing rows of `public.jwks`, behind one `*jwks.Service` that satisfies `jwtutils.KeyProvider`. The registry wraps it in `jwtutils.CachedKeyProvider` and hands the interface to the feature, so the cache is invisible to the endpoint. The document is a bare RFC 7517 JWK Set, not the API envelope; a `Source` failure degrades to the configured key, a module with no provider fails closed, and an unreadable configured key fails the run before the listener opens.
...
- `pkg/crypto` — AES-256-GCM `Cipher` (`enc:` prefix), PHC `PasswordHasher`, `KeyGenerator` emitting all four key variables, and `DecodeJWK` as the reader of the base64 JWK form the generator writes. Do not add another format.
