...
- `internal/mailer` — SMTP submission (`emersion/go-smtp` + `go-sasl`) plus the compiled React Email templates, behind one `*mailer.Service`; optional (an empty `mailer.smtp_host` builds a mailer that refuses to send); STARTTLS or implicit TLS, never plaintext silently; the error classes are `ErrNotConfigured`, `ErrNetwork`, `ErrTimeout`, `ErrCanceled`, `ErrAuth`, `ErrRejected`, `ErrTemporary`.
