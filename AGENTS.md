...
- `modules/<area>/module.go` — area module the registry loads: holds feature list and mounts each feature on one router; composition root names one module per area. Features built from area's `Deps` struct (resolved services the registry passes in).
- `modules/<area>/<feature>/schema.go` — table constants and row structs; table rename touches one line.
...
- `internal/config` file layout: one file per stage of Config's life (`types.go`, `defaults.go`, `keys.go`, `file.go`, `env.go`, `flags.go`, `validate.go`, `config.go`). Do not add a file for a single function.
